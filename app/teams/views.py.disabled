...
def mailinglist(request):
    ...
    if request.user.is_authenticated():
        ...
        teams = (
            ...
            ('TalkShops', 'talkshops', "The TalkShops working group organises educational events throughout the year to help CSE students (and anyone else, really) use CSE's technical resources, mostly the labs' computers."),
        ...
        h.request('GET', '/~csesoc/updateMailingLists?cseid=' + request.user.username)
        teams_joined = h.getresponse().read().strip().split(',')
...
def join(request, team):
    assert team in ('Beta', 'TalkShops', 'Social', 'Publicity', 'Dev', 'HS Team')
